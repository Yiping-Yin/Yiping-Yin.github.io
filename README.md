...
The hero visual is a drum of daily candlesticks read by a fixed pen. Each session is placed at the height of its own price, so a rising market lifts the ring as it goes round and the sessions on show spread over a world unit between the lowest and the highest. Candle bodies and wicks sit on an inner ring, volume bars on an outer ring, white blades run from the axis to the volume bar of the first session of each ISO week, and the same weeks carry date labels standing in vertical radial planes (a label fades in as its plane turns to the camera, so nothing reads mirrored). Around them a dial stands still in the world: one tick per slot at the volume ring and two lines marking the three-slot gap. The box is square on desktop, as imc.com's is, so the ring fills it at the 75° view the composition was drawn for. The instrument stands on the page itself, with nothing drawn under it: a key light from over the reader's left shoulder, 55° above the horizon, models the faces turned to the reader, and a cold rim light from the far side picks out their edges. The camera dollies in without ever leaving the composition the still was designed as, and then the drum steps one session clockwise per beat — the oldest session dissolving into the gap while the newest is printed under the pen and the nib slides to its close. It stops at the end of the record rather than looping. Hovering a session widens it and swaps the caption's second line; dragging rocks the drum and settles on a tick; the canvas is a slider, so the arrow, Home and End keys step through the sessions and the space bar stops and starts the beat. The composition follows the hero of imc.com; the implementation is this repository's own.

The price series is the S&P 500 index (^GSPC) daily open, high, low, close and volume for 110 NYSE sessions ending 2026-09-04, captured from Yahoo Finance and embedded in `assets/market-data.mjs` alongside its `meta` record. It is illustrative only. To swap it, replace that file with any array of `{ label, open, high, low, close, volume }` rows with an even length of at least 8 and update `meta`. Every figure in the caption — the date range, the session count, the capture date and the ×6.3 vertical exaggeration — is derived from that file and the model's `GLYPH_RATIO` by `captionLines()`, and a test asserts that the copy in `index.html` equals the same function's output for the last session, so the caption cannot drift from the data. The eye is never quite still: it swings 14° either side of the design azimuth every 48 s and leans up to 2° toward the pointer, damped over a quarter of a second so it cannot overshoot, while the pen stays fixed on the dial and the newest session stays under it — only the viewpoint drifts. The scene honours `prefers-reduced-motion` (the drum at the end of the record, fully printed, with no beat, and both the swing and the lean switched off at the exact eye the composition was designed for), remembers where it had got to in `sessionStorage` so a second visit fades up instead of performing, draws every frame the swing needs while the canvas is on screen, and stops rendering while the canvas is offscreen or the page is hidden. Phones get a shorter one-line caption, no date labels, no ticks and no lean, and draw the swing at 30 fps.
