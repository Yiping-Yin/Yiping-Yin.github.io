...
class QualityAcceptance(unittest.TestCase):
    ...
    def test_one_runtime_for_all_import_aliases(self):
        ...
        for name in ('training-copy-v1.js','training-demo-v1.js'):self.assertEqual(imports['/portfolio-assets/'+name],'/portfolio-assets/training-quality-v1.js?v=detail-1')
        self.assertIn('crossorigin src="/portfolio-assets/training-quality-v1.js?v=detail-1"',text)
        for view in ('review','market'):self.assertEqual(imports[f'/portfolio-assets/{view}-copy-v1.js'],f'/portfolio-assets/{view}-quality-v1.js'+('?v=clean-1' if view=='review' else '?v=detail-1'))
