...
def observe_native_replay(executable, url, out):
    ...
    with tempfile.TemporaryDirectory() as tmp, (out/'native-browser.log').open('w') as log:
        ...
        try:
            ...
            result.update(manual_hidden_paused=None, manual_returned_paused=None, manual_explicit_resume=None)
            if result['paused']:
                # The manual tape has a separate controller; check real tabs there too.
                send('Page.navigate', {'url':url.split('#')[0]+'#/market'}, session)
                manual_play = 'document.querySelector(\'button[aria-label="Play the tape"]\')'
                manual_pause = 'document.querySelector(\'button[aria-label="Pause the tape"]\')'
                wait(manual_play+'!==null')
                evaluate(manual_play+'.click()'); wait(manual_pause+'!==null')
                send('Target.activateTarget', {'targetId':other}); wait('document.hidden===true')
                time.sleep(.6)
                result['manual_hidden_paused'] = evaluate(manual_play+'!==null')
                send('Target.activateTarget', {'targetId':target}); wait('document.hidden===false')
                time.sleep(.6)
                result['manual_returned_paused'] = evaluate(manual_play+'!==null')
                if result['manual_returned_paused']:
                    evaluate(manual_play+'.click()'); wait(manual_pause+'!==null')
                    result['manual_explicit_resume'] = True
                    evaluate(manual_pause+'.click()')
